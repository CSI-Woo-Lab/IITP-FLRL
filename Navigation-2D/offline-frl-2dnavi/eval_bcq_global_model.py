...
n_epoch = 100
file = np.load(
    "forl_logs/model_20220104235444_bcq_Navi-Acc-Lidar-Obs-Task0_easy-v0_50_round_models/round-50-weights.npz",
    allow_pickle=True,
)
# print(file.files)
# print(file["arr_1"])
...
policy_param, q_param, imitator_param = (
    weights[:policy_len],
    weights[policy_len : policy_len + q_len],
    weights[policy_len + q_len :],
)
...
imitator_params_dict = zip(agent.impl._imitator.state_dict().keys(), imitator_param)
imitator_state_dict = OrderedDict({k: torch.tensor(v) for k, v in imitator_params_dict})
