...
def client_smooth(global_model_param, client_model_param):
    new_parameter = OrderedDict()
    for key in global_model_param.keys():
        new_parameter[key] = 0.5 * global_model_param[key].to("cuda") + 0.5 * client_model_param[key]
...
def main(log_path, env_id):
    ...
    class PPOClient(fl.client.NumPyClient):
        ...
        def set_parameters(self, parameters):
            ...
            state_dict = client_smooth(state_dict, update_dict['policy'])
